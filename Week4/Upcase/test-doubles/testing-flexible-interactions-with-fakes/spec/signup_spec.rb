  ...
  describe '#save' do
    context 'with a valid account and user' do
      it 'creates an account with one user' do
        ...
        expect(logger.messages[:debug].first).to eq('Created user user@example.com with account Example')
    ...
    context 'with an invalid account' do
      it 'logs an error message' do
        ...
        expect(logger.messages[:fatal]).to eq ['Name is already taken']
    ...
    context 'with an invalid user' do
      it 'logs an error message' do
        ...
        expect(logger.messages[:fatal]).to eq ['Email is already taken']
        expect(result).to be(false)
      end
    end
  end
